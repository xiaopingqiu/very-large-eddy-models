...
#include "VLEScoherentStructureModel.H"
...
defineTypeNameAndDebug(VLEScoherentStructureModel, 0);
addToRunTimeSelectionTable(RASModel, VLEScoherentStructureModel, dictionary);
...
tmp<volScalarField> VLEScoherentStructureModel::F2() const
{
...
VLEScoherentStructureModel::VLEScoherentStructureModel
(
...
tmp<volSymmTensorField> VLEScoherentStructureModel::R() const
{
...
tmp<volSymmTensorField> VLEScoherentStructureModel::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> VLEScoherentStructureModel::divDevReff(volVectorField& U) const
{
    return
    (
...
tmp<fvVectorMatrix> VLEScoherentStructureModel::divDevRhoReff
(
...
bool VLEScoherentStructureModel::read()
{
    if (RASModel::read())
    {
...
void VLEScoherentStructureModel::correct()
{
    RASModel::correct();
    ...
    else if (nD == 2)
    {
        ...
        Lc.internalField() = Cx().internalField()*sqrt(mesh_.V()/dimensionedScalar("thickness", dimLength, thickness));
    }
    else
    {
